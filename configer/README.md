...
- `~/.config/particlede/session.env`：保存 ParticleDE 会话主题（`GTK_THEME`/`GTK_ICON_THEME`）

说明：对 `autostart` 只会更新 `### ParticleDE CONFIG CENTER BEGIN/END` 标记中的内容，不会覆盖你已有的其它自定义。

## 主题从哪里来？如何生效？

- 主题安装位置：安装脚本会把仓库 `themes/` 下的主题复制到 `~/.themes/`（按用户安装）。
- GTK 主题生效时机：配置中心写入 `~/.config/particlede/session.env`，会在下次登录 ParticleDE 会话时由 `particlede-session` 读取并生效。
- Openbox 主题生效：如果所选主题目录包含 `openbox-3/`，配置中心会同步更新 `~/.config/openbox/rc.xml` 的主题名；然后点击“重载 Openbox”即可立即生效。
