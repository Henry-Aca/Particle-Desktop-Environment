...
def read_simple_env(path: Path) -> Dict[str, str]:
    """Parse a very small subset of shell env files: KEY=VALUE with optional quotes."""
    if not path.exists():
        return {}
    out: Dict[str, str] = {}
...
def write_particlede_session_env(*, gtk_theme: str, icon_theme: str = "Papirus-Dark") -> None:
    _ensure_parent(PARTICLEDE_SESSION_ENV)
    content = (
    ...
    _write_text(PARTICLEDE_SESSION_ENV, content)


def get_openbox_theme_name(rc_xml_path: Path) -> Optional[str]:
    if not rc_xml_path.exists():
        return None
    import xml.etree.ElementTree as ET
...
class ConfigCenter(Gtk.Application):
    ...
    def _build_common_tab(self) -> Gtk.Widget:
        ...
        current_gtk = current_env.get("GTK_THEME")
        if not current_gtk:
            current_gtk = "Arc-Dark"

        # Ensure current value is selectable
        if current_gtk and current_gtk not in themes:
            self.theme_combo.append_text(current_gtk)
            themes.append(current_gtk)

        if current_gtk and themes:
            try:
        ...
        theme_hint = Gtk.Label(
            label=(
                "说明：主题来自 ~/.themes（安装脚本会把仓库 themes/ 复制到这里）。\n"
                "GTK 主题会在下次登录 ParticleDE 时生效；Openbox 主题若该主题包含 openbox-3，则可立即重载生效。"
            ),
            xalign=0,
        )
    ...
    def _on_apply(self, *_):
        # 0) Theme (persist for next login; optionally apply Openbox theme)
        ...
        if selected_theme:
            try:
                ...
                self._show_message(f"写入 ParticleDE 会话主题失败：{exc}", Gtk.MessageType.ERROR)
                return

            # Update Openbox theme only if the theme ships openbox-3
            if theme_has_openbox(THEMES_DIR, selected_theme):
                try:
                    set_openbox_theme_name(OPENBOX_RC, selected_theme)
                except FileNotFoundError:
                    # rc.xml missing is already handled later; keep behavior consistent
                    pass
                except Exception as exc:
                    self._show_message(f"写入 Openbox 主题失败：{exc}", Gtk.MessageType.ERROR)
                    return
        ...
        extra = ""
        if selected_theme:
            extra = "；主题已保存（GTK 主题需下次登录生效）"
        self._show_message("已保存：快捷键 / autostart / 面板设置" + extra)
