...
def apply_theme_tint2(themes_dir: Path, theme_name: str, dest_tint2rc: Path = TINT2_RC) -> None:
    """Merge theme-provided tint2rc into user's tint2rc if present.

    Rules:
    - Keys present in theme tint2rc override user's values.
    - Keys not present in theme tint2rc keep user's values.
    - Panel geometry (panel_position/panel_size) is never overridden by theme.
    """
    ...
    updates = read_kv_config(src)
    updates.pop("panel_position", None)
    updates.pop("panel_size", None)

    ensure_parent(dest_tint2rc)
    if dest_tint2rc.exists():
        update_kv_config(dest_tint2rc, updates)
        return

    # If user tint2rc doesn't exist, write only the theme keys (excluding geometry).
    content = "# Generated by ParticleDE Config Center (theme tint2 merge)\n"
    for k in sorted(updates.keys()):
        content += f"{k} = {updates[k]}\n"
    write_text(dest_tint2rc, content)
